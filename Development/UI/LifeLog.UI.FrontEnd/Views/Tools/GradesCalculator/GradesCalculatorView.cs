...
namespace LifeLog.UI.FrontEnd.Views.Tools.GradesCalculator
{
	public partial class GradesCalculatorView : DevExpress.XtraEditors.XtraUserControl
	{
		...
		private void CalculateGrades()
		{
			...
			if (somaPesos == 0)
			{
				AppHelper.StatusMessage("A soma dos pesos é zero. Verifica os dados.", false);
				return;
			}

			if (somaPesos > 100)
			{
				AppHelper.StatusMessage($"A soma dos pesos ultrapassa 100% ({somaPesos}%). Corrige os valores.", false);
				return;
			}
			...
			AppHelper.StatusMessage("Notas calculadas!", true);
		}
