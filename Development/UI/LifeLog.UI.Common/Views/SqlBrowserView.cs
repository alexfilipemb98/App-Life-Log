		...
		private void bbiExecute_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
		{
			...
			try
			{
				...
				foreach (string query in queries)
				{
					...
					if (isSelectQuery)
					{
						IEnumerable<dynamic> results = AppSession.DataEngine.SQL.LoadData<dynamic>(query.Trim());
					...
					{
						int affectedRows = AppSession.DataEngine.SQL.ExecuteScalar<int>(query.Trim());
						dynamic expandoObj = new ExpandoObject();
						IDictionary<string, object> expandoDict = expandoObj;
