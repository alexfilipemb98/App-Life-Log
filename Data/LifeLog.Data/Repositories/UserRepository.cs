...
public class UserRepository : BaseRepository<User>, IUserRepository
{
    ...
    public UserRepository(UnitOfWork db, SqlDataAccess sql) : base(db, sql)
    {
    }
    ...
    {
        bool exists = await _db.Query<UserXpo>().AnyAsync(w => w.Id == key);
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    public async Task<(User?, string)> GetByKey(Guid id)
    {
        UserXpo? user;
        user = await _db.GetObjectByKeyAsync<UserXpo>(id);
    ...
    {
        List<UserXpo> users = await _db.Query<UserXpo>().ToListAsync();
        bool found = users.Count > 0;
        List<User?>? result = users.ConvertAll(u => u.ToModel());
    ...
    /// </summary>
    /// <returns></returns>
    public async Task<(User?, string)> GetLast()
    {
        User? user = await base.GetLastInsert();
    ...
    /// <returns></returns>
    public async Task<(bool, string)> Save(User model)
    {
    ...
    /// <param name="key"></param>
    /// <returns></returns>
    public async Task<(User?, string)> Duplicate(Guid key)
    {
        UserXpo existingUser = await _db.GetObjectByKeyAsync<UserXpo>(key);
        ...
        UserXpo? duplicateUser = await _db.GetObjectByKeyAsync<UserXpo>(existingUser.Id);
    ...
    public async Task<(bool, string)> RegisterUser(string username, string email, string password)
    {
        ...
        UserXpo newUser = new UserXpo(_db)
        {
            Id = Guid.NewGuid(),
            Username = username,
            Email = email,
    ...
    /// <param name="email"></param>
    /// <returns></returns>
    public async Task<(User?, string)> UserByEmail(string email)
    {
        UserXpo? user;
        user = await GetUserByEmailHelper(email);
        bool found = user is not null;
    ...
    /// <param name="email"></param>
    /// <returns></returns>
    private async Task<UserXpo?> GetUserByEmailHelper(string email)
    {
        UserXpo? user = await _db.Query<UserXpo>().FirstOrDefaultAsync(w => w.Email == email);
