	...
	public partial class CommandsView : XtraUserControl
	{
		...
		private async void bbiSave_ItemClick(object sender, ItemClickEventArgs e)
		{
			try
			{
				...
				if (saved)
				{
					List<CommandsDTO> lista = bsCommandsList.List.Cast<CommandsDTO>().ToList();
		...
		private async void bbiImport_ItemClick(object sender, ItemClickEventArgs e)
		{
			try
			{
				using (var openFileDialog = new OpenFileDialog())
				{
					...
					{
						List<CommandsDTO> commandsJson = JsonUtil.ImportFromFile<List<CommandsDTO>>(openFileDialog.FileName);
		...
		{
			if (!(tileView.GetFocusedRow() is CommandsDTO model))
				return;
		...
		private void tileView_CustomItemTemplate(object sender, TileViewCustomItemTemplateEventArgs e)
		{
			try
			{
				CommandsDTO model = ControlsHelper.GetObjectByRowHandle<CommandsDTO>(tileView, e.RowHandle);
		...
		/// <returns></returns>
		private void ExecuteFile(CommandsDTO command, bool runInAdmin = false)
		{
		...
		/// </summary>
		/// <param name="command"></param>
		private void ShowDetailView(CommandsDTO command = null)
		{
			try
			{
				if (command == null)
				{
					command = new CommandsDTO();
		...
		{
			List<CommandsDTO> list = bstiShowDisabledCommands.Checked ? _listCommands : _listCommands.Where(w => w.IsEnabled).ToList();

			if (!(listboxPrograms.SelectedItem is ExternalProgramsDTO ep) || ep.Id == Guid.Empty)
		...
		private bool ValidateModel()
		{
			...
			if (cbeProgram.GetSelectedDataRow() is ExternalProgramsDTO program)
				_crtCommands.ExternalProgram = program;
			else if (!_crtCommands.IsEnabled)
				dxErrorProvider.SetError(cbeProgram, _crtCommands.GetValidationMessage(nameof(CommandsDTO.ExternalProgram)));

			if (string.IsNullOrWhiteSpace(_crtCommands.Name))
				dxErrorProvider.SetError(teName, _crtCommands.GetValidationMessage(nameof(CommandsDTO.Name)));

			if (string.IsNullOrWhiteSpace(_crtCommands.Command))
				dxErrorProvider.SetError(recMain, _crtCommands.GetValidationMessage(nameof(CommandsDTO.Command)));

			if (string.IsNullOrWhiteSpace(_crtCommands.Description))
				dxErrorProvider.SetError(teDescription, _crtCommands.GetValidationMessage(nameof(CommandsDTO.Description)));
		...
		private void tileView_ItemDrop(object sender, ItemDropEventArgs e)
		{
			Task.Run(async () =>
			{
				...
				for (int i = 0; i < tileView.RowCount; i++)
				{
					...
					if (tileView.GetRow(rowHandle) is CommandsDTO command)
					{
						//command.Position = i;
			...
			CommandsDTO cmd = tileView.GetObjectByRowHandle<CommandsDTO>(e.RowHandle);
